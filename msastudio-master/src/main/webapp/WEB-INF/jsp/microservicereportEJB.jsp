...
<%--	<table id="example"
		class="display  table table-hover table-striped table-responsive">
		<thead class="thead-inverse">
			<tr>
				<th width="3%">#</th>
				<th width="13%">Business Usecase</th>
				<th width="12%">Component Type</th>
				<th width="12%">Business Functionality</th>
				<th width="12%">Referenced Functionality</th>
				<th width="21%">Call Sequence</th>
				...
				<c:forEach var="method" items="${report.methods}">
					<c:if test = "${method.loc > 0}">
						<tr>
							...
							<td><c:out value="${report.sterioType}" /></td>
										...
										<c:forEach var="hierarchy" items="${method.methodCallHierarchy}">
											...
											<c:choose>
												<c:when test="${hierarchy.objectRefDefineType eq 'user_defined'}">
													...
													<td>
														<c:if test="${fn:length(classNames) gt 1}">
    ...
    <script src="js/microreport/app_ejb.js"></script>	
	</div>
