...
app.controller('MainCtrl', ['$scope', '$http', '$log', function ($scope, $http, $log) {
	...
	$scope.tId=$("#myDiv").attr("data-my-variable2");
	  ...
	  $scope.gridOptions.columnDefs = [
		    ...
		    { name: 'Business Usecase',field:"className", width: '70%' ,cellTemplate: '<div>' +
  ...
  $http.get('integrationReportJSON?projectId='+$scope.pId+'&technology='+$scope.tId)//
	        ...
	        for(j = 0; j < methods.length; j++){
	        	methods[j].subGridOptions = {
		          columnDefs: [ {name:"Id", field:"id", width: '5%'},        	 
		        	  ...
		        	  {name:"Call Sequence", field:"methodSequence", width:'35%', wordWrap: true }, //, cellTemplate:'<div>'+$scope.parseSequence('{{COL_FIELD}}')+'</div>'		        	 
		        	  {name:"Integration Component", field:"integrationComp", width: '25%'},
		        	  {name:"Technology", field:"technology", width: '25%'}
		        	  ],
